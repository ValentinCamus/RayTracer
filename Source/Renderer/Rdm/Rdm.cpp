...
namespace rt
{
    float rt::rdm::Chiplus(float c) { return (c > 0.f) ? 1.f : 0.f; }

    float rt::rdm::Beckmann(float ndoth, float alpha)
    {
        ...
        float denom = float(M_PI) * alpha2 * Pow2(ndoth2);
        return (numi / denom);
    }

    bool rt::rdm::Snell(float ldoth, float n1, float n2, float & cosThetaT)
    {
    ...
    float rt::rdm::Fresnel(float cosThetaI, float n1, float n2)
    {
        float cosThetaT;
    ...
    float rt::rdm::G1(float ddoth, float ddotn, float alpha)
    {
    ...
    float rt::rdm::Smith(float ldoth, float ldotn, float vdoth, float vdotn, float alpha)
    {
    ...
    color3 rt::rdm::BsdfSpecular(float ldoth, float ndoth, float vdoth, float ldotn, float vdotn, Material * m)
    {
        const float b = Beckmann(ndoth, m->Roughness());
        const float f = Fresnel(ldoth, 1.0f, m->IndexOfRefraction());
    ...
    color3 rt::rdm::BasdfDiffuse(Material * m) { return (m->Diffuse() / (float) M_PI); }

    color3 rt::rdm::Bsdf(float ldoth, float ndoth, float vdoth, float ldotn, float vdotn, Material * m)
    {
